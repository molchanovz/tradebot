...
func GetReadyFile(wildberriesKey, supplyId string) error {
	...
	for _, order := range orders {
		...
		ordersSlice = append(ordersSlice, "wb_stickers_fbs/ready/"+strconv.Itoa(order.ID)+".pdf")
	}
	err := mergePDFsInDirectory(ordersSlice, "wb_stickers_fbs/"+supplyId+".pdf")
	if err != nil {
		return err
	}
	if !fileExists("wb_stickers_fbs/" + supplyId + ".pdf") {
...
func decodeToPNG(base64String string, orderId int) string {
	...
	filePath := "wb_stickers_fbs/codes/" + strconv.Itoa(orderId) + ".png" // Замените на желаемое имя файла и расширение
...
func decodeToPDF(base64String string, orderId int, order OrderWB) {
	...
	for _, barcode := range order.SKUs {
		skuImageUrl = "wb_stickers_fbs/barcodes/" + barcode + ".png"
	...
	if skuImageUrl == "" {
		...
		skuImageUrl = "wb_stickers_fbs/generated/" + order.Article + "_generated.png"
		err := createBarcodeWithSKU(order.Article, skuImageUrl, 40)
		if err != nil {
			log.Printf("Ошибка при создании изображения с артикулом: %v", err)
			skuImageUrl = "wb_stickers_fbs/barcodes/0.png" // Резервный пустой баркод
		}
	}

	pdf.ImageOptions(skuImageUrl, (75-58)/2, 67, 58, 40, false, gofpdf.ImageOptions{ImageType: "PNG"}, 0, "")
	// Сохранение PDF-документа
	err := pdf.OutputFileAndClose("wb_stickers_fbs/ready/" + strconv.Itoa(orderId) + ".pdf")
	if err != nil {
...
func Clean_files(supplyId string) {
	err := os.RemoveAll("wb_stickers_fbs/codes")
	if err != nil {
		fmt.Println(err)
	}
	err = os.RemoveAll("wb_stickers_fbs/ready")
	if err != nil {
		fmt.Println(err)
	}
	err = os.Mkdir("wb_stickers_fbs/codes", 0755) // 0755 - это права доступа к директории (чтение, запись, выполнение)
	if err != nil {
		fmt.Println(err)
	}
	err = os.Mkdir("wb_stickers_fbs/ready", 0755) // 0755 - это права доступа к директории (чтение, запись, выполнение)
	if err != nil {
		fmt.Println(err)
	}
	err = os.Remove("wb_stickers_fbs/" + supplyId + ".pdf")
	if err != nil {
		fmt.Println(err)
	}
